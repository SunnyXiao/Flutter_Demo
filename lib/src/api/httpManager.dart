...
import 'package:flutter_app/src/api/httpError.dart';
import 'package:flutter_app/utils/logUtil.dart';

/// 参考来源：https://github.com/Cheney2006/flutter_utils/blob/master/lib/http/http_manager.dart
/// https://juejin.im/post/5e6dcfc7e51d45270411837d#heading-11
///http请求成功回调
...
class HttpManager {
  ...
  static final HttpManager _instance = new HttpManager._internal();
  ...
  void init({String baseUrl, int connectTimeout, int receiveTimeout, List<Interceptor> interceptors}) {
    _client.options =
        _client.options.merge(baseUrl: baseUrl, connectTimeout: connectTimeout, receiveTimeout: receiveTimeout);
  ...
  void _request({
    ...
    try {
      ...
      Response<Map<String, dynamic>> response =
          await _client.request(url, data: data, queryParameters: params, options: options, cancelToken: cancelToken);
      String statusCodes = response.data["statusCode"];
      int statusCode = response.statusCode;
      if (statusCode == 200 || statusCodes == '0') {
        //成功
        ...
        if (errorCallback != null) {
          errorCallback(HttpError(statusCodes, message));
        }
      }
  ...
  Future<T> _requestAsync<T>({
    ...
    try {
      ...
      Response<Map<String, dynamic>> response =
          await _client.request(url, queryParameters: params, data: data, options: options, cancelToken: cancelToken);
      String statusCode = response.data["statusCode"];
      if (statusCode == "0" || response.statusCode == 200) {
        //成功
        if (jsonParse != null) {
  ...
  void download({
    ...
    options ??=
        options == null ? Options(receiveTimeout: receiveTimeout) : options.merge(receiveTimeout: receiveTimeout);
